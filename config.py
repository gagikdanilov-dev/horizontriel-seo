...
# ВАЖНО: это отдельный адрес от Тильды.
SITE_BASE = os.environ.get(
    "SITE_BASE",
    "https://catalog.horizontriel.com"
).rstrip("/")

# Главный сайт (Тильда) — основная брендовая/SEO-страница
MAIN_SITE = os.environ.get(
    "MAIN_SITE",
    "https://horizontriel.com"
).rstrip("/")
...
# ── Основной SEO-регион ──
SEO_REGION = "Ставропольский край"
...
def slugify(text):
    """Кириллица/латиница → безопасный slug: mineralnye-vody, stavropolskiy-kray."""
    if not text:
        return ""
...
TYPE_META = {
    ...
    "Коммерция": {
        ...
        "plural": "Коммерческая недвижимость",
        "gen": "коммерческую недвижимость",
        "genpl": "объектов",
    },
    "Гараж": {
        "slug": "garazhi",
...
# Для них SEO-страница создаётся всегда.
# Пустые страницы должны оставаться noindex и не попадать в sitemap.
PRIORITY_TYPES = [
    "Квартира",
    "Дом",
    "Земля",
...
# Только Ставропольский край / КМВ.
PRIORITY_CITIES = [
    "Кисловодск",
    "Пятигорск",
    "Ессентуки",
...
# ── Предложный падеж городов для SEO-текстов ──
CITY_PREP = {
...
def type_meta(t):
    return TYPE_META.get(
        t,
        {
            "slug": slugify(t) or "obekty",
            "plural": t or "Объекты",
            "gen": (t or "объект").lower(),
            "genpl": "объектов",
        },
    )
